  ...
  switch ($normalizedTask) {
    ...
    "frontend-implementation" {
      return [PSCustomObject]@{
        ...
        RoutingReason = "Frontend implementation routed to K2"
        RequiresCoordinatorDecision = $false
      }
    }
    "frontend-styling" {
      return [PSCustomObject]@{
        TargetModel = $FrontendModel
        ModelRole = "frontend-specialist"
        RoutingReason = "Frontend styling routed to K2"
        RequiresCoordinatorDecision = $false
      }
    }
    "frontend-bugfix" {
      return [PSCustomObject]@{
        TargetModel = $FrontendModel
        ModelRole = "frontend-specialist"
        RoutingReason = "Frontend bugfix routed to K2"
        RequiresCoordinatorDecision = $false
      }
    }
...
$requiresStrategy = "no"
$model = ""
$flashModel = "google/antigravity-gemini-3-flash"
$k2Model = "kimi-for-coding/k2p5"
$frontendModel = "kimi-for-coding/k2p5"
$coordinatorModel = "google/antigravity-claude-opus-4-5-thinking"
...
$promptFile = "$reportFile.prompt.txt"
Remove-Item -Path $rawFile -Force -ErrorAction SilentlyContinue
...
while ($attempt -le $maxAttempts) {
  ...
  if ($opencodeCmd) {
    $opencodeExecutable = $opencodeCmd.Source
  }
  ...
  $cmdArgs.Add("--file") | Out-Null
  $cmdArgs.Add($promptFile) | Out-Null
  $cmdArgs.Add("--") | Out-Null
  $cmdArgs.Add("Read the attached instruction file and return ONLY the required BEGIN_REPORT..END_REPORT block.") | Out-Null

  try {
    $proc = Start-Process `
      -FilePath $opencodeExecutable `
    ...
    if (-not $proc.WaitForExit($timeoutSec * 1000)) {
      ...
      $runExit = if ($null -eq $proc.ExitCode) { 1 } else { [int]$proc.ExitCode }
    }
  } catch {
    $runExit = 1
  }
  ...
  $hasContract = [regex]::IsMatch($combined, "(?ms)^BEGIN_REPORT\r?\n.*?^END_REPORT\s*$")
  if ($hasContract) {
    $runExit = 0
    break
  }
...
if ($isGitRepo) {
  ...
  if ($promptFile.StartsWith($workdir, [System.StringComparison]::OrdinalIgnoreCase)) {
    $promptRel = $promptFile.Substring($workdir.Length).TrimStart("\", "/") -replace "\\", "/"
  }
  ...
  foreach ($changed in $delta) {
    ...
    if ($promptRel -and $changed -eq $promptRel) { continue }
